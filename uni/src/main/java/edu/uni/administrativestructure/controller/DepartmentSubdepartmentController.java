...
public class DepartmentSubdepartmentController {
     ...
     * @throws IOException
     */
    @ApiOperation(value = "根据学校id来查询一二级部门关系", notes = "已测试")
    @ApiImplicitParams({
            @ApiImplicitParam(name="university_id", value = "学校id", required = true, dataType = "bigint", paramType = "path"),
    })
    @GetMapping(value = "/departmentSubdepartments/listByUniversityId/{university_id}")
    public void listByUniversityID(@PathVariable long university_id,
        ...
        String json = cache.get(cacheName);
        if(json == null){
            List<DepartmentSubdepartment> departmentSubdepartments = departmentSubdepartmentService.selectPageByUniversity( university_id);
            json = Result.build(ResultType.Success).appendData("departmentSubdepartments", departmentSubdepartments).convertIntoJSON();
     ...
     * @throws IOException
     */
    @ApiOperation(value = "根据一级部门id来查询一二级部门关系", notes = "已测试")
    @ApiImplicitParams({
            @ApiImplicitParam(name="department_id", value = "一级部门id", required = true, dataType = "bigint", paramType = "path"),
    })
    @GetMapping(value = "/departmentSubdepartments/listByDepartmentId/{department_id}")
    public void listByDepartmentID(@PathVariable long department_id,
                                   HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListByDepartmentId_CacheNamePrefix + department_id;
        String json = cache.get(cacheName);
        if(json == null){
            List<DepartmentSubdepartment> departmentSubdepartments= departmentSubdepartmentService.selectPageByDepartment(department_id);
            json = Result.build(ResultType.Success).appendData("departmentSubdepartments", departmentSubdepartments).convertIntoJSON();
            if(departmentSubdepartments != null) {
                cache.set(cacheName, json);
            }
     ...
     * @throws IOException
     */
    @ApiOperation(value = "根据二级部门id来查询一二级部门关系", notes = "已测试")
    @ApiImplicitParams({
            @ApiImplicitParam(name="subdepartment_id", value = "二级部门id", required = true, dataType = "bigint", paramType = "path"),
    })
    @GetMapping(value = "/departmentSubdepartments/listBySubdepartmentId/{subdepartment_id}")
    public void listBySubdepartmentID(@PathVariable long subdepartment_id,
                                   HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListBySubdepartmentId_CacheNamePrefix + subdepartment_id;
        String json = cache.get(cacheName);
        if(json == null){
            List<DepartmentSubdepartment> departmentSubdepartments = departmentSubdepartmentService.selectPageBySubdepartment( subdepartment_id);
            json = Result.build(ResultType.Success).appendData("departmentSubdepartments", departmentSubdepartments).convertIntoJSON();
            if(departmentSubdepartments != null) {
                cache.set(cacheName, json);
            }
