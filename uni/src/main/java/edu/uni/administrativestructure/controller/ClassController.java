...
public class ClassController {
    ...
    static class CacheNameHelper{
        ...
        private static final String ListAll_CacheName="as_class_listAll_";
        private static final String ListByCode = "as_class_listByCode_";
        private static  final String ListBySpecialty="as_class_listByCode_";
        private static final String ListLikeName_name="as_class_listLikeName_";
    }
    /**
     * 新增班级信息
     * @param class_1
    ...
    public Result create(@RequestBody(required = false) Class class_1){
        if(class_1 != null){
            ...
            if(success){
                ...
                cache.deleteByPaterm(CacheNameHelper.ListByCode + "*");
                cache.deleteByPaterm(CacheNameHelper.ListBySpecialty + "*");
                cache.deleteByPaterm(CacheNameHelper.ListLikeName_name+"*");
     ...
     * @throws IOException
     */
    @ApiOperation(value = "根据班级编号获取班级详情", notes = "已测试")
    @ApiImplicitParam(name = "code", value = "班级id", required = true, dataType = "varchar", paramType = "path")
    @GetMapping("classByCode/{code}")
    public void selectByCode(@PathVariable String code,
                        HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListByCode + code;
        String json = cache.get(cacheName);
        if(json == null){
            Class class_1= classService.selectByCode(code);
            json = Result.build(ResultType.Success).appendData("class", class_1).convertIntoJSON();
            if(class_1 != null){
                cache.set(cacheName, json);
            }
     ...
     * @throws IOException
     */
    @ApiOperation(value="根据班级名称模糊查询获取班级详情", notes="已测试")
    @ApiImplicitParam(name = "name", value = "班级名称", required = false, dataType = "String", paramType = "path")
    @GetMapping("/class/listLikeName/{name}")
    public void receive(@PathVariable String name, HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListLikeName_name + name;
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success).appendData("class",classService.selectLikeName(name)).convertIntoJSON();
            cache.set(cacheName, json);
        }
        response.getWriter().write(json);
    }
     ...
     * @throws IOException
     */
    @ApiOperation(value = "根据学校id和页码来查询班级信息", notes = "已测试")
    @ApiImplicitParams({
            @ApiImplicitParam(name="university_id", value = "学校id", required = true, dataType = "bigint", paramType = "path"),
    })
    @GetMapping(value = "/classes/listByUniversityId/{university_id}")
    public void listByUniversityID(@PathVariable long university_id,
                                   HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListByUniversityId_CacheNamePrefix + university_id;
        String json = cache.get(cacheName);
        if(json == null){
            List<Class> classes = classService.selectPageByUniversity(university_id);
            json = Result.build(ResultType.Success).appendData("classes", classes).convertIntoJSON();
            if(classes != null) {
                cache.set(cacheName, json);
            }
        }
     ...
     * @throws IOException
     */
    @ApiOperation(value = "根据一级部门id和页码来查询班级信息", notes = "已测试")
    @ApiImplicitParams({
            @ApiImplicitParam(name="department_id", value = "一级部门id", required = true, dataType = "bigint", paramType = "path"),
    })
    @GetMapping(value = "/classes/listByDepartmentId/{department_id}")
    public void listByDepartmentID(@PathVariable long department_id,
                                   HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListByDepartmentId_CacheNamePrefix;
        String json = cache.get(cacheName);
        if(json == null){
            List<Class> classes = classService.selectPageByDepartment( department_id);
            json = Result.build(ResultType.Success).appendData("classes", classes).convertIntoJSON();
            if(classes != null) {
                cache.set(cacheName, json);
            }
    ...
    @GetMapping(value = "/classes/listAll")
    public void list(HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListAll_CacheName;
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success).appendData("classes", classService.selectAll()).convertIntoJSON();
            cache.set(cacheName, json);
        }
        response.getWriter().write(json);
    }
     ...
     * @throws IOException
     */
    @ApiOperation(value = "根据专业id来查询班级信息", notes = "已测试")
    @ApiImplicitParams({
            @ApiImplicitParam(name="specialty_id", value = "二级部门id", required = true, dataType = "bigint", paramType = "path"),
    })
    @GetMapping(value = "/classes/listBySpecialtyId/{specialty_id}")
    public void listBySpecialty(@PathVariable long specialty_id,
                                      HttpServletResponse response) throws IOException {
        response.setContentType("application/json;charset=utf-8");
        String cacheName = CacheNameHelper.ListBySpecialty + specialty_id;
        String json = cache.get(cacheName);
        if(json == null){
            List<Class> classes= classService.selectSpecilty(specialty_id);
            json = Result.build(ResultType.Success).appendData("classes", classes).convertIntoJSON();
            if(classes != null){
                cache.set(cacheName, json);
            }
