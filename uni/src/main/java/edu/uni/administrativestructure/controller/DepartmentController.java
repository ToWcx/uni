...
public class DepartmentController {
    ...
    static class CacheNameHelper{
        ...
        public static final String ListLikeName_name="as_department_selectLikeName_";
    }
     ...
     * @throws IOException
     */
    @ApiOperation(value="根据部门名称模糊查询获取部门详情", notes="已测试")
    @ApiImplicitParam(name = "name", value = "部门名称", required = false, dataType = "String", paramType = "path")
    @GetMapping("/department/listLikeName/{name}")
    public void receive(@PathVariable String name, HttpServletResponse response) throws IOException {
        ...
        if(json == null){
            json = Result.build(ResultType.Success).appendData("department",departmentService.selectLikeName(name)).convertIntoJSON();
    ...
    @GetMapping(value = "/departments/listByUniversityId/{university_id}")
    public void listByUniversityID(@PathVariable long university_id,
        ...
        String json = cache.get(cacheName);
        if(json == null){
            List<Department> departments = departmentService.selectPageByUniversity(university_id);
            json = Result.build(ResultType.Success).appendData("departments", departments).convertIntoJSON();
            if(departments != null) {
                cache.set(cacheName, json);
            }
        }
