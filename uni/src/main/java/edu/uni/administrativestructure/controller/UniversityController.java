...
public class UniversityController {
    ...
    public void receive(@PathVariable String name, HttpServletResponse response) throws IOException {
        ...
        String json = cache.get(cacheName);
        if(json == null){
            json = Result.build(ResultType.Success).appendData("university",  universityService.selectLikeName(name)).convertIntoJSON();
            cache.set(cacheName, json);
        }
        response.getWriter().write(json);
    }
     ...
     * @throws IOException
     */
//    @ApiOperation(value="excel导出所有学校", notes="未测试")
